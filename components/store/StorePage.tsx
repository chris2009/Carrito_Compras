        ...
        <section className="px-4 py-10">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="flex gap-3 overflow-x-auto pb-2 snap-x">
              {categories.map((cat) => (
                <Link key={cat.id} href={`/productos?categoria=${cat.slug}`} className="flex-shrink-0 snap-start">
        ...
        <section className="px-4 py-10">
          <div className="mx-auto max-w-6xl">
            <div className="mb-4 flex items-center justify-between">
              ...
              <Link href="/productos?destacado=true" className="flex items-center gap-1 text-sm hover:underline" style={{ color: store.theme.primary }}>
                Ver todos <ArrowRight className="h-3 w-3" />
              </Link>
            </div>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
              {featured.map((product) => (
                <ProductCard key={product.id} product={product} currency={store.currency} primaryColor={store.theme.primary} />
              ))}
            </div>
          </div>
        </section>
        ...
        <section className="bg-gray-50 px-4 py-10">
          <div className="mx-auto max-w-6xl">
            <div className="mb-4 flex items-center justify-between">
              ...
              <Link href="/productos?orden=newest" className="flex items-center gap-1 text-sm hover:underline" style={{ color: store.theme.primary }}>
